...
from unrealsdk import make_struct, find_all, find_object, find_class #type: ignore
...
def qtd_pressed(obj: UObject, args: WrappedStruct, _3: Any, func: BoundFunction) -> type[Block] | None:
    ...
    if args.ChoiceNameId == "QuitToDesktop": # quit to desktop button
        with prevent_hooking_direct_calls():
            ...
            func(find_object("object", "Engine/Transient.BPMenu_GFxDialogWindow_C_0"), "GbxMenu_Secondary1", make_struct("GbxMenuInputEvent"))
        return Block
    else: # does nothing if cancel button is pressed
        return

def get_location_and_save():
    global spawn_location, spawn_rotation, station, map_name
...
build_mod(options=[button_override_option], hooks=[])
